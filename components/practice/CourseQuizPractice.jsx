...
export default function CourseQuizPractice() {
  ...
  const timerValueSec = quizSubmitted
    ? typeof frozenTimerSec === 'number'
      ? frozenTimerSec
      : 0
    : timeLeftSec ?? 0
  const isTimerCritical = mode === 'quiz' && hasQuizParams && timerMinutes > 0 && timerValueSec <= 60
  const isTimerWarning =
    mode === 'quiz' && hasQuizParams && timerMinutes > 0 && timerValueSec > 60 && timerValueSec <= 300
  const timerToneClass = isTimerCritical
    ? 'border-rose-300 bg-rose-50 text-rose-900'
    : isTimerWarning
      ? 'border-amber-300 bg-amber-50 text-amber-900'
      : 'border-sky-200 bg-sky-50 text-sky-900'
  const timerLabel = isTimerCritical
    ? 'Critical: less than a minute'
    : isTimerWarning
      ? 'Ending soon'
      : 'Time remaining'
        ...
        <header className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {mode === 'review' ? (
                <button
                  ...
                  className="rounded-xl border border-teal-300 bg-teal-50 px-3.5 py-2 text-xs font-semibold text-teal-800 transition hover:bg-teal-100"
                >
                  Switch to quiz mode
                </button>
              ) : (
            ...
            {mode === 'quiz' && hasQuizParams && timerMinutes > 0 ? (
              <div
                className={`w-full rounded-2xl border px-4 py-3 shadow-sm sm:w-auto sm:min-w-[220px] ${timerToneClass} ${
                  isTimerCritical && !quizSubmitted ? 'animate-pulse' : ''
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[11px] font-semibold uppercase tracking-wide">{timerLabel}</span>
                  {isTimerCritical && !quizSubmitted ? (
                    <AlertTriangle className="h-4 w-4 shrink-0" aria-hidden />
                  ) : null}
                </div>
                <div className="mt-1 flex items-center gap-2 font-mono text-2xl font-bold tabular-nums">
                  <Clock className="h-5 w-5 shrink-0 opacity-80" aria-hidden />
                  {formatCountdown(timerValueSec)}
                </div>
                <p className="mt-1 text-[11px] font-medium opacity-80">
                  {quizSubmitted ? 'Timer stopped after submission' : 'Auto-submits when timer hits 00:00'}
                </p>
              </div>
            ) : null}
          </div>
                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <input
                          ...
                          className="h-9 min-w-0 flex-1 rounded-md border border-slate-300 px-2 text-sm sm:min-w-[240px]"
                        />
                        <button
                          type="button"
                          onClick={async () => {
